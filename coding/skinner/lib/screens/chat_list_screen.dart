...
/// Screen that lists all 1-to-1 chat channels for the logged-in user.
/// Each chat channel is created when a patient pays for an appointment.
...
class _ChatListScreenState extends State<ChatListScreen> {
  ...
  Future<void> _fetchChats() async {
    ...
    try {
      ...
      setState(() {
        _errorMessage = 'Failed to load chats. Pull down to retry.';
      });
    } finally {
  ...
  /// Get the display name for the other party in the chat.
  String _getChatTitle(Map<String, dynamic> chat) {
    return chat['doctor_name'] ??
        chat['patient_name'] ??
  ...
  String _formatLastTime(Map<String, dynamic> chat) {
    ...
    if (lastMsg is Map) {
      dateStr =
          lastMsg['created_at']?.toString() ?? lastMsg['createdAt']?.toString();
    }
      ...
      return const Center(
        child: Column(
          ...
          children: [
            ...
            SizedBox(height: 8),
            Text(
              'Book an appointment and pay to start\nchatting with your doctor.',
              textAlign: TextAlign.center,
